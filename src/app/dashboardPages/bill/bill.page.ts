...
export class BillPage implements OnInit {
  ...
  async ionViewDidEnter() { 
    ...
    this.route.queryParams.subscribe(async params => {
      console.log("nav param...",params.transctionId)
      if (params.transctionId) {
        ...
        console.log("else trans id...",this.transactionID)
        ...
        this.dashboardService.energyConsumedDetails(dataObj).subscribe(data => {
          ...
          this.cost = parseFloat(this.kwh) * parseFloat(stationDetailsInfo.pricingDetails);
          this.energyConsumed_Time = data.timeSpentCharging
  ...
  async generateInvoice() {
    ...
    dataObj = {
      "siteId": stationDetailsInfo.siteID, "stationId": stationinfo.stationId, "transactionId": this.transactionID,
  ...
  async makePaymentUsingWallet() {
    this.paymentButtonCreateOrderLoader = true;
    let dataObj = { "transactionId": this.transactionStatusID, "transactionAmount": this.invoiceData.grandTotal, "useWalletBalance": true, "walletAmount": this.invoiceData.grandTotal }
    this.dashboardService.makePaymentUsingWallet(dataObj).subscribe(data => {
  ...
  async makePayment() {
    ...
    this.orderId = await this.dashboardService.getItem(constString.ORDER_ID)
    this.transactionID = await this.dashboardService.getItem(constString.TRANSACTION_ID)
    let previousOrderStatus = await this.dashboardService.getObject(constString.PAYMENT_ORDER_DATA)
    this.dashboardService.getPaymentLink(this.orderId).subscribe(data => {
      this.paymentLink = data.paymentLink
    })
