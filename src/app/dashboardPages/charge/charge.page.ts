  ...
  async refreshChargedData() {
    ...
    if(this.transactionID != null) {
      ...
      this.dashboardService.getItem(constString.TRANSACTION_ID).then(data => {
        if (data) {
          ...
          this.dashboardService.getActiveTransactionForUser().subscribe(data => {
            if(data.chargingStatus == null || data.chargingStatus == "NotCharging" && data.transactionId && !data.isInvoiceAvailable) {
      ...
      this.dashboardService.getActiveTransactionForUser().subscribe(data => {
        ...
        if(error.chargingStatus == null || error.chargingStatus == "NotCharging" && !error.transactionId) {
        ...
        // if(error.chargingStatus == "Charging" && error.transactionId) {
        //   this.stopClicked = "false";
        //   this.btnUnlock = true;
        // }
    ...
    this.dashboardService.startCharging(dataObjStart).subscribe(data => {
      if (data.hasError === false) {
        ...
        if(data.response.status == 1) {
          ...
          this.loaderService.hideLoader();
          this.alertService.presentAlertPopup('Power consumption not detected. Please check cable connection','app/station-details');
        }
      }
      else {
